...
* Fixed conventional calculator email recipient logic never using custom email
* Fixed wp_kses_post called on array in FHA, MHA, and RC email templates
* Fixed VA funding fee e_rate uninitialized for 1-4% down payment range
* Fixed refinance calculator monthly savings truncated by parseInt on locale string
* Fixed copy-paste bug using wrong network settings option key for VA calculator
* Fixed redirect page slug mismatch in network options update
* Fixed typo in network options permission error message
